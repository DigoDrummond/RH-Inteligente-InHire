# -*- coding: utf-8 -*-
"""Aplica migration 075 com filtros de empresa e workflow"""
import sys
import os
...
try:
    print("\n=== APLICANDO MIGRATION 075 ===\n")

    # Ler migration
    with open('migrations/075_add_filters_empresa_workflow.sql', 'r', encoding='utf-8') as f:
        sql = f.read()
    ...
    print("✅ Migration 075 aplicada com sucesso!")
    ...
    # Exemplo
    result = session.execute(text("""
        SELECT talent_name, vaga_nome, conhecia_framework
        FROM vw_relatorio_candidaturas
        WHERE conhecia_framework IS NOT NULL
        LIMIT 5
    """))
    ...
    for row in result.fetchall():
        print(f"   - {row[0]}: {row[1]}")
        print(f"     Framework? {row[2] or 'N/A'}")
