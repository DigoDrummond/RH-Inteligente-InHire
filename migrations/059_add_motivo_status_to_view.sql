/*
================================================================================
MIGRATION 059: Adicionar campo motivo_status (CORRIGIDA)
================================================================================
...
OBJETIVO:
  ...
  Adicionar nova coluna 33 na view vw_analise_posicoes: motivo_status

  Este campo extrai o campo "notes" da última mudança de status em position_timeline.

NOVO CAMPO:

  - motivo_status (coluna 33, no final)
  - Extrai: ultimo_status_posicao.notes
  - Cobertura: ~67% das posições (2.418/3.618 eventos)
  - Exemplos: waiting_schedule, feedback_received_from_client, strategy_change

Base: Migration 057 (mantém TODOS os 32 campos anteriores)
...
ultimo_status_posicao AS (
    -- ✨ ADICIONADO: campo 'notes' (Migration 059)
    SELECT DISTINCT ON (posicao_id)
        posicao_id,
        new_status,
        changed_at AS data_ultima_mudanca,
        notes  -- ✨ NOVO: motivo da última mudança
...
SELECT
    ...
    -- 33. ✨ Motivo do Status (NOVO - Migration 059)
...
'View FINAL para análise de posições - Migration 059 (2026-02-19).
33 campos. Histórico:
...
'Código do motivo da última mudança de status da posição. Extrai position_timeline.notes.
Exemplos: waiting_schedule, feedback_received_from_client, strategy_change, closed_other_vendor.
Cobertura: ~67% das posições (2.418/3.618 eventos). (Migration 059)';
