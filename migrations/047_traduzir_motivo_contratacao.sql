-- Migration 047: Traduzir campo motivo_contratacao para Português
-- Data: 2026-02-10
-- Motivo: Usuário reportou que "Aumento de quadro" não aparece na posição 931
-- Descoberta: Campo está em posicoes.reason com valor 'expansion' (código em inglês)
-- Solução: Traduzir codes da API para descrições em português

-- ANTES:
-- posicoes.reason = 'expansion'  ->  motivo_contratacao = 'expansion'
--
-- DEPOIS:
-- posicoes.reason = 'expansion'  ->  motivo_contratacao = 'Aumento de quadro'
...
SELECT
    ...
    -- 13. Motivo de contratação (TRADUZIDO PARA PORTUGUÊS)
    -- MUDANÇA DESTA MIGRATION:
    -- Antes: p.reason AS motivo_contratacao
    -- Depois: CASE para traduzir códigos inglês -> português
    CASE p.reason
    ...
    -- 27. Meta Recrutamento (indicador_prazo)
    CASE
...
COMMENT ON VIEW vw_analise_posicoes IS 'View FINAL para análise de posições - Migration 047 (2026-02-10). 27 campos. Campo motivo_contratacao traduzido: expansion→Aumento de quadro, replacement→Substituição, other→Outros.';

-- VALIDAÇÃO
-- Para verificar se a migration funcionou:
-- SELECT id_position, cargo, motivo_contratacao FROM vw_analise_posicoes WHERE id_position = 931;
-- Resultado esperado: motivo_contratacao = 'Aumento de quadro'

-- DISTRIBUIÇÃO DOS MOTIVOS:
-- SELECT motivo_contratacao, COUNT(*) FROM vw_analise_posicoes
-- WHERE motivo_contratacao IS NOT NULL GROUP BY 1 ORDER BY 2 DESC;
-- Aumento de quadro: ~437 posições (54.6%)
-- Substituição: ~330 posições (38.1%)
-- Outros: ~63 posições (7.3%)
