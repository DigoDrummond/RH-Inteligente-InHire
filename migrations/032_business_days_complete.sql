/*
================================================================================
MIGRATION 032: Implementação Completa de Dias Úteis (CONSOLIDADA)
================================================================================

Data: 2026-02-06
Descrição:
  Migration consolidada que implementa TUDO necessário para dias úteis:
  1. Tabela de feriados (nacionais e MG)
  2. Função calcular_dias_uteis
  3. Materialized View com performance otimizada

  Esta migration substitui e consolida as migrations 030 e 031.

Impacto:
  - Cria tabela feriados
  - Cria função de cálculo de dias úteis
  - Converte view para materialized (performance)
  - Todos os SLAs em dias úteis

Performance:
  - Consultas instantâneas (< 50ms)
...
COMMENT ON TABLE feriados IS 'Feriados nacionais e de MG para cálculo de dias úteis';
...
-- ============================================================================
-- PARTE 2: FUNÇÃO CALCULAR DIAS ÚTEIS
-- ============================================================================

-- Remover se já existir
DROP FUNCTION IF EXISTS calcular_dias_uteis(DATE, DATE) CASCADE;

-- Criar função (STABLE porque consulta tabela feriados)
...
COMMENT ON FUNCTION calcular_dias_uteis IS 'Calcula dias úteis excluindo finais de semana e feriados';
...
COMMENT ON MATERIALIZED VIEW vw_analise_posicoes IS 'View analítica com SLAs em DIAS ÚTEIS. Performance otimizada. Para atualizar: REFRESH MATERIALIZED VIEW vw_analise_posicoes;';
...
BEGIN
    ...
    -- Testar função (janeiro/2024: 22 dias úteis)
    ...
    RAISE NOTICE '================================================================================';
    RAISE NOTICE 'MIGRATION 032 - DIAS ÚTEIS COMPLETO';
    RAISE NOTICE '================================================================================';
    RAISE NOTICE 'Total de posições: % (esperado: 830)', v_total_posicoes;
    RAISE NOTICE 'Total de feriados: %', v_total_feriados;
    RAISE NOTICE '';
    RAISE NOTICE 'Teste função dias úteis (Jan/2024): % (esperado: 22)', v_teste_dias_uteis;
    RAISE NOTICE 'Posição 241 - SLA Geral: % dias úteis', COALESCE(v_sla_241::TEXT, 'NULL');
    RAISE NOTICE '';
    RAISE NOTICE 'Implementado:';
    RAISE NOTICE '  OK Tabela feriados criada';
    RAISE NOTICE '  OK Função calcular_dias_uteis criada';
    RAISE NOTICE '  OK Materialized view criada';
    RAISE NOTICE '  OK 5 índices criados';
    RAISE NOTICE '';
