/*
================================================================================
MIGRATION 055: Remover fallback problemático que usa candidaturas.vaga_id
================================================================================
...
PROBLEMA IDENTIFICADO:
  ...
  A Migration 054 introduziu fallback usando candidaturas.vaga_id que causa
  DUPLICAÇÃO de candidatos em vagas com múltiplas posições.

  EXEMPLO DO BUG:
  ---------------
  Vaga: "FRAMEWORK PADAWANS - EPISÓDIO VI 🪐"
  - 41 posições com talent_ids únicos
  - 8 posições: talent_id existe em talentos → nome correto ✅
  - 33 posições: talent_id NÃO existe em talentos → fallback ativa
  - Fallback usa WHERE vaga_id → retorna sempre "Bruno Gomes Ferreira"
  - RESULTADO: Bruno aparece 34x (1 correto + 33 do fallback)

  CAUSA RAIZ:
  -----------
  Migration 054 (linhas 248-271) tem fallback que faz:

  SELECT ... FROM candidaturas cd2
  WHERE cd2.vaga_id = p.vaga_id  -- ❌ PROBLEMA: JOIN por vaga
    AND cd2.stage_name = 'Contratação'
  ORDER BY cd2.updated_at_inhire DESC
  LIMIT 1

  Quando múltiplas posições têm talent_id NULL (ou não existem em talentos),
  TODAS recebem o MESMO candidato (último updated_at da vaga).

SOLUÇÃO:

  REMOVER o fallback problemático. Se talent_id não existe na tabela talentos,
  o campo deve ser NULL (correto), não deve buscar candidato genérico da vaga.

  Benefícios:
  1. Elimina duplicação indevida de candidatos
  2. Mostra dados apenas quando há certeza (via talent_id)
  3. NULL indica "dados não disponíveis" (correto)
  4. Mais simples e performático

PRÓXIMOS PASSOS:

  Após aplicar esta migration, sincronizar talent_ids faltantes:
  - Identificar talent_ids em posicoes que não existem em talentos
  - Buscar dados desses talentos na API InHire
  - Inserir na tabela talentos
  - Resultado: 100% das posições com dados corretos
...
-- ============================================================================
-- 1. RECRIAR VIEW REMOVENDO FALLBACK PROBLEMÁTICO
-- ============================================================================
...
SELECT
    ...
    -- 25. Nome da Pessoa Contratada (CORRIGIDO - Migration 055)
    -- ✅ REMOVIDO fallback problemático que usava vaga_id
    -- Se talent_id não existe em talentos, retorna NULL (correto)
    t_contratado.name AS nome_pessoa_contratada,

    -- 26. E-mail Pessoal (CORRIGIDO - Migration 055)
    -- ✅ REMOVIDO fallback problemático que usava vaga_id
    -- Se talent_id não existe em talentos, retorna NULL (correto)
...
FROM posicoes p
    ...
    -- ✅ JOIN direto usando posicoes.talent_id (Migration 054)
...
'View FINAL para análise de posições - Migration 055 (2026-02-19).
31 campos. Histórico:
...
com talentos. Se talent_id não existe na tabela talentos, retorna NULL (correto). Fallback
problemático que causava duplicação foi removido. (Migration 055)';
