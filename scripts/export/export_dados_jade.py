"""
Script para exportar vw_dados_jade para Google Sheets usando OAuth2
Exporta dados customizados para Jade (689 registros)
"""
import os
import sys
import pickle
...
def export_to_sheet(service, spreadsheet_id: str, sheet_name: str, columns: list, data: list, batch_size: int = 5000):
    ...
    try:
        ...
        print(f"   [Lote 0/{total_batches}] Escrevendo cabecalho...")
        ...
        for batch_num in range(total_batches):
            ...
            print(f"      OK - {cells_updated:,} celulas atualizadas")

        print(f"\n   TOTAL - {total_cells_updated:,} celulas atualizadas em {total_batches} lotes\n")
...
def main():
    ...
    try:
        print("\n" + "="*70)
        print(" EXPORTACAO vw_dados_jade PARA GOOGLE SHEETS")
        print("="*70 + "\n")
        ...
        print(f"View: vw_dados_jade")
        print(f"Aba destino: API_Dados_Jade\n")
        ...
        print("3. Exportando vw_dados_jade -> API_Dados_Jade")
        print("-" * 70)
        print("   Buscando dados da view (pode levar alguns segundos)...")

        columns, data = fetch_view_data(conn, 'vw_dados_jade')
