"""
Script para exportar vw_funil_performance para Google Sheets usando OAuth2
Exporta dados de performance do funil de candidaturas (85k+ registros)
"""
import os
import sys
import pickle
...
def export_to_sheet(service, spreadsheet_id: str, sheet_name: str, columns: list, data: list, batch_size: int = 10000):
    """
    ...
    Args:
        ...
        batch_size: Tamanho do lote para escrita (padrão: 10000 linhas)
    """
    try:
        ...
        print(f"   (Aguarde, pode levar 3-6 minutos para ~85k registros...)\n")
...
def main():
    ...
    try:
        print("\n" + "="*70)
        print(" EXPORTACAO vw_funil_performance PARA GOOGLE SHEETS")
        print("="*70 + "\n")

        # URL e ID da planilha
        spreadsheet_url = "https://docs.google.com/spreadsheets/d/1pWscZVbQ_jA7D5aJWycDuRi--8M_AIPSDN9j451-Pd0"
        spreadsheet_id = "1pWscZVbQ_jA7D5aJWycDuRi--8M_AIPSDN9j451-Pd0"

        print(f"Planilha: {spreadsheet_id}")
        print(f"View: vw_funil_performance")
        print(f"Aba destino: Funil_API\n")
        ...
        # Exporta vw_funil_performance
        print("3. Exportando vw_funil_performance -> Funil_API")
        print("-" * 70)
        print("   Buscando dados da view (pode levar alguns segundos)...")

        columns, data = fetch_view_data(conn, 'vw_funil_performance')
