...
# Autenticar na API
print("\n[2] Autenticando na API Inhire...")

auth_url = f"{settings.INHIRE_AUTH_URL}login"
auth_data = {
...
try:
    with httpx.Client(timeout=30.0) as client:
        ...
        if auth_response.status_code != 200:
            print(f"ERRO: Falha na autenticação (status {auth_response.status_code})")
            sys.exit(1)
        ...
        api_url = f"{settings.INHIRE_BASE_URL}jobs/positions/paginated/{vaga_inhire_id}"
        ...
        if response.status_code != 200:
            print(f"ERRO: Falha ao buscar posições (status {response.status_code})")
        ...
        if not posicao_api:
            ...
            print(f"Total de posições retornadas: {len(positions)}")
            sys.exit(1)
        ...
        if timeline:
            print("\n  Todos os eventos (ordenados):")
            eventos_ordenados = sorted(timeline, key=lambda x: x.get('changedAt', ''), reverse=True)
